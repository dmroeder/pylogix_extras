...
with pylogix.PLC("192.168.1.10") as comm:
    ...
    axis_instance = 0x01
    alarm_attribute = [0x1a]
    fault_attribute = [0x19]

    # get the fault log for the axis
    combined_records = []
    ret = comm.Message(0x03, 0xb1, axis_instance, fault_attribute)
    if ret.Status == "Success":
        combined_records.extend(get_records(ret.Value, True))

    # get the alarm log for the axis
    ret = comm.Message(0x03, 0xb1, axis_instance, alarm_attribute)
    if ret.Status == "Success":
